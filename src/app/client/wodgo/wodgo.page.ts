...
import { Workout, Exercise, Exercises } from 'src/app/models/wod.model';
...
@Component({
  ...
  animations: [
    trigger('fadeIn', [
      ...
      transition('false <=> true', animate(300)), // Set the animation duration
    ]),
  ],
})
export class WodgoPage implements OnInit {
  ...
  fetchExerciseInfo(workout: any, exerciseNameKey: string) {
    const exeName = workout[exerciseNameKey];

    if (exeName) {
      this.dataService.getExebyname(exeName).subscribe((exercise) => {
        // Add exercise information to the workout
        workout[exerciseNameKey] = exercise;
      });
    }
  }
